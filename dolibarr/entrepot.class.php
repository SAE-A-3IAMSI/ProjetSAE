...
class Entrepot extends CommonObject
{
	...
	public function fetch($id, $ref = '')
	{
		...
		if ($result) {
			if ($this->db->num_rows($result) > 0) {
				...
				$warehouseStockInfo = $this->get_warehouse_stock_info();
				
				// Add stock information to the object
				$this->warehouse_stock_info = $warehouseStockInfo;
  	...
  	public function get_warehouse_stock_info()
	{
		// Build SQL query to get warehouse stock information
		$sql = "SELECT ps.fk_product, ps.reel";
		...
		if ($result === false) {
			...
			$this->error = $this->db->lasterror();
			dol_syslog("Erreur SQL : " . $this->error, LOG_ERR);
			return array('error' => $this->error);
		}

		$warehouseStockInfo = array();

		// Fetch warehouse stock data and store it in an array
		while ($row = $this->db->fetch_array($result)) {
			$warehouseStockInfo[$row['fk_product']] = array(
				'name' => $this->get_name_product_by_id($row['fk_product']),
				'reel' => $row['reel'],
			);
		}
	...
	public function get_name_product_by_id($id)
	{
		$sql = "SELECT ref";
